...
{
    // VStack tier 1: a plain Panel that stacks its children via the shared VStackEngine.
    // The engine owns all the layout/spring logic (see VStackEngine); this host is just the
    // plain-panel chrome plus the property/event forwarding.
    [Designer(typeof(VStackDesigner))]
    public class vStackPanel : Panel, IVStackHost
    {
        private readonly VStackEngine _engine;

        public vStackPanel()
        {
            // Engine first: setting Padding below triggers a layout pass, and OnLayout forwards
            // to the engine — it must exist before that fires.
